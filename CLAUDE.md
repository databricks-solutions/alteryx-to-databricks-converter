...
- Test suite: ~1373 tests; CLI has 11 commands (`convert`, `analyze`, `portfolio`, `validate`, `verify`, `sync`, `advise`, `profile`, `list-tools`, `plugins`, `version`); `tests/unit/verification/` covers the harness and `tests/fixtures/expected_outputs/` holds golden CSVs.
