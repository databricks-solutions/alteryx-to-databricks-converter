...
- **Frontend**: React SPA served by the FastAPI server (the Settings page no longer has a default-output-format selector — every conversion produces all five formats).
...
Different migration targets and team preferences. The CLI and server both produce all five formats per conversion by default; reviewers can compare them side-by-side and pick the one that fits the deployment story.
